...
def verify(request, auth_token):
    try:
        ...
        if profile_obj:
         ...
         # If profile_obj is None (invalid token), handle gracefully
...
def search_view(request):
    q = request.GET.get('q','')
    if q:
        results = productModel.objects.filter(productname__icontains=q) # Search by product name
    else:
        results = productModel.objects.none()  # No search term = No results

    return render(request, 'User/search_results.html', {'results': results, 'q': q})
